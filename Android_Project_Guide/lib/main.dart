  ...
  Future<void> _generateAndShare() async {
    if (_textController.text.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('请输入一些文字')),
      );
      return;
    }
    ...
    try {
      ...
      // 3. 绘制文字
      // 计算垂直居中位置
      final double x = _textArea.left + (_textArea.width - bestPainter.width) / 2;
      final double y = _textArea.top + (_textArea.height - bestPainter.height) / 2;
      ...
      if (byteData != null) {
        ...
        final file = await File('${tempDir.path}/sketchbook_share.png').create();
